...
inline void lawicel_timer_tick() {
	...
	while (ring_buffer_available(rx)) {
		if (ring_buffer_find(rx, '\r')) {
			...
			case LAWICEL_GET_HARDWARE:
				reply_size = sprintf((char*) reply_data, "vSTM32\r");
				break;

			case LAWICEL_GET_VERSION:
				reply_size = sprintf((char*) reply_data, "V%02d%02d\r",
				UART2CAN_VERSION_HW, UART2CAN_VERSION_SW);
				break;
